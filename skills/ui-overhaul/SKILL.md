---
name: ui-overhaul
description: "Audit and consolidate a web app's UI: measure visual drift (hardcoded colors, ad-hoc font sizes, duplicated spacing literals, hand-maintained dark-mode twins, N spellings of one component, hand-rolled copies of the UI library's own button/input re-typed inline, reaching past your own wrapper to the library's raw component, one composite rebuilt from library calls in N files, the same kind of object presented as a modal on one screen and a panel on another, inconsistent action-row order, alignment and commit contract, inconsistent or doubled focus rings and hover states, controls styled for the mouse but not the keyboard), establish a semantic design-token layer, migrate the markup to it, unify duplicated UI into shared primitives, and run hierarchy and polish passes per surface. Also audits the project's own design rules (CLAUDE.md / AGENTS.md) and guard tests in both directions — too strict (rules that veto design ideas) or too loose (rules that sound binding but catch nothing). Use this skill whenever the user wants to clean up, unify, or systematize an app's UI, design, or styling — phrases like 'make the design consistent', 'the UI is a mess', 'every screen looks different', 'clean up the frontend', 'extract a design system', 'design tokens', 'dark mode is painful to maintain', 'UI audit', 'consolidate components', 'make it look professional', 'polish pass', 'the focus ring looks wrong', 'hover behaves differently on every screen', or 'should this be a modal or a panel' — and when they ask whether their design rules or guard tests have gone too far or aren't pulling their weight. Also trigger when a restyle or redesign request reveals organic growth (many one-off styles) rather than one bad screen. Works best on Tailwind projects; includes guidance for plain CSS."
---
...
1. **Scan for drift:**
   ...
   ```
   This measures: raw palette utilities bypassing tokens, hand-maintained dark twins, arbitrary values duplicated across files, the inline font-size ladder, tracking values, raw shadows, hex colors in CSS; then interactive state — the spellings of each hover/focus property, sites that opt out of the focus system, and transitions that name only colors while a state also moves a shadow or transform; then what the markup *is* rather than how it looks: control geometry spelled inline (grouped by signature, so two files sharing one surface as a component spelled twice), controls styled for the mouse only, and shared primitives with almost no call sites; and finally the same question one rung higher, where a component library hides it — **wrappers reached past** (a primitive that re-presents a library component, against the count of files using that component raw) and **repeated library composites** (the same run of library calls with the same salient props, in 2+ files). Read the flagged sites before reporting — counts are signal, not verdicts.

   **A quiet scan is a finding, not a pass.** The value checks can all read zero on a codebase that is still substantially hand-built — every color a token, every size on the ladder — because a hand-rolled copy of your button library is made *entirely of correct utilities*, and a footer that rebuilds your own save bar is made of entirely correct components. Duplication hides one layer above wherever you last looked: values → controls → **library calls** → composites. The last five sections chase it up that ladder, and steps 3–5 below carry it further than any regex can.

   **Read adoption and "reached past" as one number.** A primitive with 2 call sites is a lead; 2 adopted against 6 files hand-rolling the thing it wraps is a finding with a size, a migration list, and a sentence for the report. The scan pairs them for exactly this reason — it was added after a codebase where the adoption list correctly flagged `ui/Modal.vue` at 2 call sites while 6 other files overrode the library modal's content slot, each re-declaring the panel's radius, background and overflow and rebuilding the icon+title header the wrapper existed to end (measured: that header was 40px and round in three of them, 32px on an 8px radius in a fourth). The lead was on the report; the number that makes someone act on it was not.
...
3. **Inventory the idioms.** Start from the scan's control-geometry clusters and its two library-level checks — those are the ones a grep would have to be lucky to find — then count spellings for the idioms it cannot see: section labels (how many combinations of size/weight/uppercase/tracking say "this is a section"?), page headers, empty states, avatars, field rows, menus. Five spellings of one idiom is one component waiting to exist.

   **Inventory the overlay forms too, and this one is not about how things look.** Every app of size has several containers an object can appear in — a centered dialog, a side panel, a full page, an inline expansion — and drifts by presenting *the same kind of object* in two of them. That is invisible to every check above, because each container is internally consistent; the defect is the mapping. List every overlay in the app, what object each presents, and which form it uses, then ask two questions:
   - **Is the form chosen per object, or per interaction?** "Cards open in a panel; everything else is a dialog" is a rule a user learns once. "Viewing is a panel, editing is a dialog" makes them learn the same object twice, and it is how one object ends up with two implementations that then drift apart. Per-object is also the version you can check.
   - **What does each form actually buy here?** A side panel's usual argument is that the surface behind it stays *usable* — but most overlay panels are modal, which freezes that surface and leaves only the visual half of the benefit. Check with the real thing (is the background inert? does it carry `aria-hidden`?) rather than from the pattern's reputation. Both Atlassian and Shopify deprecated their modal drawer; the panels that thrived are part of the layout, not overlays. A modal panel can still be right — seeing where a record sits while you read it is worth something — but write down which half you are buying, because the next person will otherwise re-derive it as a gap.

   Whichever way it lands, the answer belongs in the rules document as a lookup ("this object gets this form"), not as a judgment call at each new surface.
   ...
   **Take the action row seriously — it is the most-repeated composite in most apps and the one nobody names.** Every modal footer, every inline editor commit, every delete confirmation is the same object: a primary, a way out, sometimes a destructive escape and a status line. It drifts on four axes that no value scan can see, so check each one explicitly across *every* such row before concluding anything:
   ...
   - **Whether there is a primary at all — the commit contract.** Does this surface commit per field as you set it, or stage everything behind a Save? Both are defensible, and the axis is which one *this* surface claims. Check it per surface and then check it *within* each one, because the worst version is internal: a settings dialog where adding a column applies instantly (it needs an id back from the server) while renaming and filtering wait for Save is two contracts in one panel, and its "discard unsaved changes?" prompt is then a lie about half of what the user just did — the columns are already gone. Where the object being edited already exists, per-field commit usually wins and takes the whole footer with it; a transaction is what creating and destroying need. That split makes the chrome derivable instead of argued: no Save on an editor, one commit on a wizard, a confirmation on a delete.
...
4. **Inventory the states, the same way.** Hover, focus, active and disabled drift exactly like colors do — N spellings of one thing, maintained per call site — and nothing in a static reading of the markup shows it. Three questions, in this order:
   ...
   - **Does each overlay and icon-only control say what it is?** A separate axis from focus and easy to miss beside it, because it is not a *style* — nothing renders differently and no scan sees it. Two specific holes worth checking directly: a dialog that hides its own header (or overrides the content slot wholesale) usually loses its accessible name with it and is announced as nothing at all; and a two-step confirmation that swaps a labelled button for a tick and a cross replaces one named control with two unnamed ones, exactly where the stakes went up. Check the overlays with the accessibility inspector, not by reading the markup — the library supplies the name from a `title` prop the hand-rolled sites skipped.
...
"none — this is genuinely bespoke"); every wrapper reached past, as the pair
"N adopted against M hand-rolled"; the repeated library composites; and the
action row on its five axes: order, alignment, hint placement, icon on the
primary, commit contract]

## Overlay forms
[Every overlay, the object it presents, and its form (dialog / panel / page /
inline). Whether the mapping is per object or per interaction, and for any
panel: modal or part of the layout — measured, not assumed]
...
"reachable and unmarked" — only the second is a defect. Plus overlays and
